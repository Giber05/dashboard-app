...
export type AddCustomerController = {
...
function useAddCustomerHandler(): AddCustomerController {
  const dispatch = useAppDispatch();
  const { authUser } = useSelector(selectAuth);
  const { isActionLoading, isActionModalVisible } = useSelector(selectCustomerAction);
  const addCustomerUC = new AddCustomer();
  ...
  const onFormSubmitted = (values: { name: string; address: string; country: string; phone_number: string; job_title: string; status: boolean }) => {
    ...
    setTimeout(async () => {
      const resource = await addCustomerUC.execute({ token: authUser?.accessToken!, name: values.name, address: values.address, country: values.country, status: values.status, jobTitle: values.job_title, phoneNumber: values.phone_number });
...
export default useAddCustomerHandler;
